...
int main(int argc, char *argv[]) {
    int i, ch, keyQueue, queueId, semId, shmId, cpt = 0, chunk;
    int differentChunks[9], *unique, offset, length;
    ...
    /* Get the ids from the controller */
    ...
    /* Draw the map and load the state window */
    drawMap(gameWindow, semId, map);
    loadStateWindowGame(stateWindow);

    posX = (unsigned char) (map->posX * SQUARE_WIDTH);
    posY = map->posY;

    /* Determine the chunk the player is currently in */
    chunk = ((((int) posY / CHUNK_SIZE) + 1) * ((((int) posX / SQUARE_WIDTH) / CHUNK_SIZE) + 1)) - 1;
    ...
    while (run && (ch = getch()) != KEY_F(2)) {
        offset = posY * MAP_WIDTH * sizeof(unsigned char) + (posX / SQUARE_WIDTH) * sizeof(unsigned char);

        for (i = 0; i < 8; ++i) {
            differentChunks[i] = getChunkAt(offset, i);
        }
        ...
        /* Lock every chunk adjacent to the current position */
        for (i = 0; i < length; ++i) {
        ...
        if (map->livesLeft > 0) {
            ...
            if (ch == ERR) {
                /* We redraw the entire map so we unlock all the semaphores that we had locked */
                for (i = 0; i < length; ++i) {
