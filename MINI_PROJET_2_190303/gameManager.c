...
bool isMinotaurNearby(map_t *map) {
    ...
    || (rightDown < MAP_WIDTH * MAP_HEIGHT && rightDown % MAP_WIDTH != 0 && map->map[rightDown] >= MINOTAUR_ID_BEGIN)) {
        /* Minotaur is on a square adjacent to us */
...
int getNextOffset(int currOff, int direction) {
    ...
    switch (direction) {
        ...
        case LEFT_UP:
            off = getNextOffset(currOff, UP) - 1;
            break;

        case LEFT_DOWN:
            off = getNextOffset(currOff, DOWN) - 1;
            break;
...
int getChunkAt(int currOff, int direction) {
    ...
    nextOff = getNextOffset(currOff, direction);

    switch (direction) {
        case UP:
            chunk = (nextOff >= 0) ? ((((nextOff / MAP_WIDTH) / CHUNK_SIZE) + 1) * (((nextOff % MAP_WIDTH) / CHUNK_SIZE) + 1)) - 1 : -1;
            break;

        case DOWN:
            chunk = (nextOff < MAP_WIDTH * MAP_HEIGHT) ? ((((nextOff / MAP_WIDTH) / CHUNK_SIZE) + 1) * (((nextOff % MAP_WIDTH) / CHUNK_SIZE) + 1)) - 1 : -1;
            break;

        case RIGHT:
            chunk = (nextOff < MAP_WIDTH * MAP_HEIGHT && nextOff % MAP_WIDTH != 0) ? ((((nextOff / MAP_WIDTH) / CHUNK_SIZE) + 1) * (((nextOff % MAP_WIDTH) / CHUNK_SIZE) + 1)) - 1 : -1;
            break;

        case LEFT:
            chunk = (nextOff >= 0 && nextOff % MAP_WIDTH != (MAP_WIDTH - 1)) ? ((((nextOff / MAP_WIDTH) / CHUNK_SIZE) + 1) * (((nextOff % MAP_WIDTH) / CHUNK_SIZE) + 1)) - 1 : -1;
            break;
        ...
        case LEFT_DOWN:
            chunk = (nextOff < MAP_WIDTH * MAP_HEIGHT && nextOff % MAP_WIDTH != (MAP_WIDTH - 1)) ? ((((nextOff / MAP_WIDTH) / CHUNK_SIZE) + 1) * (((nextOff % MAP_WIDTH) / CHUNK_SIZE) + 1)) - 1 : -1;
            break;

        case RIGHT_UP:
            chunk = (nextOff >= 0 && nextOff % MAP_WIDTH != 0) ? ((((nextOff / MAP_WIDTH) / CHUNK_SIZE) + 1) * (((nextOff % MAP_WIDTH) / CHUNK_SIZE) + 1)) - 1 : -1;
            break;
