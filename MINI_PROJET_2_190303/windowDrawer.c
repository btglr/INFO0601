...
void drawSquare(WINDOW *window, int type, int x, int y, bool refresh) {
    ...
    if (type >= MINOTAUR_ID_BEGIN && type <= MINOTAUR_ID_END) {
        color = COLOR_PAIR(PAIR_COLOR_MINOTAUR);

        wattron(window, color);
        wattron(window, WA_BOLD);
        mvwaddch(window, y, x, MINOTAUR_CHAR);
        mvwaddch(window, y, x + 1, MINOTAUR_CHAR);
...
void drawMap(WINDOW *window, int semId, map_t *map) {
    ...
    for(i = 0; i < MAP_WIDTH * MAP_HEIGHT; ++i) {
        ...
        chunk = (((y / CHUNK_SIZE) + 1) * (((x / SQUARE_WIDTH) / CHUNK_SIZE) + 1)) - 1;

        P(semId, SEM_MAP_CHUNK + chunk, 1);
        drawSquare(window, map->map[i], x, y, false);
