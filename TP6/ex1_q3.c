...
int main(int argc, char *argv[]) {
    ...
    if (argc >= 4) {
        ...
        if (action == 'E') {
            if (argc >= 5) {
                msg = (char*) malloc(strlen(argv[4]) + 1);
                ...
                fprintf(stderr, "At least 4 arguments are required\n");
                exit(EXIT_FAILURE);
            }
        }
    }
    ...
    switch (action) {
        case 'R':
            ...
            if (msgrcv(messageId, &request, sizeof(request_t) - sizeof(long), request.type, 0) == -1) {
