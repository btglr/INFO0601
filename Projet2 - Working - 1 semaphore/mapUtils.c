...
int getWallCount(map_t *map, int type) {
    ...
    for(i = 0; i < MAP_WIDTH * MAP_HEIGHT; ++i) {
        switch (type) {
            case INVISIBLE_WALL:
                wallCount = (map->map[i] == INVISIBLE_WALL) ? wallCount + 1 : wallCount;
                break;

            case DISCOVERED_WALL:
                wallCount = (map->map[i] == DISCOVERED_WALL) ? wallCount + 1 : wallCount;
                break;

            case VISITED_SQUARE:
                wallCount = (map->map[i] == VISITED_SQUARE) ? wallCount + 1 : wallCount;
                break;

            default:
                wallCount = (map->map[i] == VISIBLE_WALL || map->map[i] == INVISIBLE_WALL) ? wallCount + 1 : wallCount;
                break;
        }
    }
